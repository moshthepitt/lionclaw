...
3. long-polls `/v0/channels/stream/pull` for progress events,
4. starts Telegram typing from kernel queue/runtime status events,
5. leases provider deliveries from `/v0/channels/outbox/pull`,
6. sends one Telegram message per outbox lease and reports the provider outcome to `/v0/channels/outbox/report`,
7. advances its progress cursor through `/v0/channels/stream/ack`.
...
- Telegram delivery is outbox-driven: typing comes from progress streams, final answers come from durable outbox leases, no reasoning lane delivery.
