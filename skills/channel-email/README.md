...
small ambient allowlist for browser, proxy, locale, certificate, terminal, and
temp directory behavior; arbitrary shell secrets are not inherited.
...
Setup also rejects malformed, duplicate, LionClaw-managed, or secret-bearing
OAuth query parameter names before opening the browser or storing refresh state.
Token endpoint secrets and grant fields belong in the POST body, not
`--token-url`.
...
proxy, locale, and TLS certificate settings, closes stdin, reads one UTF-8
access token from stdout, enforces a 10 second timeout plus 16 KiB stdout and
stderr caps, and surfaces bounded redacted stderr diagnostics on helper
