...
Setup also rejects malformed or secret-bearing authorization parameter names,
such as `client_secret`, plus duplicate names across the provider preset,
`--auth-url` query, and `--auth-param`.
