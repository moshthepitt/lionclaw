...
use crate::model::{
    ConversationId, EffectId, MissionId, OutputSemantics, RoleName, TaskId, TaskNamespace,
};
use crate::ports::{
    EffectCleaner, EffectCleanupRequest, ExecutionControl, RoleRunOutcome, RoleRunRequest,
    RoleRunner,
};
...
async fn await_bounded_real_turn(
    ...
    mut run: tokio::task::JoinHandle<Result<RoleRunOutcome, lionclaw_runtime_api::TypedFailure>>,
    cleanup: impl std::future::Future<Output = ()>,
) -> Result<RoleRunOutcome, lionclaw_runtime_api::TypedFailure> {
...
async fn prove_real_runtime_continuity(runtime: &str, credential_target: &Path) {
    ...
    let task_id = TaskId::new("real-runtime-continuity").unwrap();
    let role_name = RoleName::new("investigator").unwrap();
    ...
    assert_eq!(
        crate::model::role_prompt_template(
            TaskNamespace::Planning,
            OutputSemantics::ProducesReport
        ),
        Some(crate::model::RolePromptTemplate::Planning),
    ...
    assert_eq!(
        ConversationId::for_role_instance(
            &first_request.mission_id,
            first_request.namespace,
            &first_request.task_id,
            &first_request.role.name,
            first_request.assignment_epoch,
        ),
        conversation_id,
    ...
    assert_eq!(
        ConversationId::for_role_instance(
            &second_request.mission_id,
            second_request.namespace,
            &second_request.task_id,
            &second_request.role.name,
            second_request.assignment_epoch,
        ),
        conversation_id,
