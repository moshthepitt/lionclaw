...
async fn queued_input_after_a_running_boundary_resumes_the_same_generation() {
    ...
    assert_eq!(
        resumed.conversations[&conversation_id].assignment_epoch,
        assignment_epoch
    );
    assert_eq!(
...
async fn production_broadcast_is_atomic_when_one_live_conversation_is_full() {
    ...
    let mission = engine
        ...
        .unwrap();
    engine
        .propose_plan(&mission, proposal(0, work_and_independent_validator_plan()))
        .await
        .unwrap();
    approve_plan(&engine, &mission).await;
    let driver = tokio::spawn({
    ...
    let writer_id = active
        .conversations
        .iter()
        .find(|(_, conversation)| conversation.task_id.as_str() == "writer")
        .map(|(id, _)| id.clone())
        .unwrap();
    let validator_id = active
        .conversations
        .iter()
        .find(|(_, conversation)| conversation.task_id.as_str() == "validator")
        .map(|(id, _)| id.clone())
        .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        ...
        Some(&ConversationQueueFull {
            conversation_id: writer_id.clone(),
