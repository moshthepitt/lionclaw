...
type SessionObservations = Arc<Mutex<Vec<(Option<String>, bool, std::path::PathBuf)>>>;
...
fn validation_handoff(prompt: &str) -> serde_json::Value {
    let assertions = prompt
        .split_once("## Contract assertions in scope")
        .map(|(_, rest)| rest)
        .unwrap_or_default()
        .lines()
        ...
        .collect::<Vec<_>>();
    assert!(!assertions.is_empty(), "validator prompt has no assertions");
    serde_json::json!({
        "schema": "lionclaw.mission.validate-handoff.v2",
        "type": "validate",
        "done": true,
        "report": "production assertions validated",
        "items": assertions
            .into_iter()
            .map(|item_id| serde_json::json!({"item_id": item_id, "passed": true}))
            .collect::<Vec<_>>(),
        "passed": true,
...
impl DeliveryTransport {
    ...
    fn writer_work(execution: &TurnExecution) -> std::path::PathBuf {
        let mission = Self::conversation(execution)
            .parent()
            .and_then(Path::parent)
            .expect("mission state root");
        std::fs::read_dir(mission.join("tasks"))
            .unwrap()
            .filter_map(Result::ok)
            .map(|entry| entry.path().join("work"))
            .find(|path| path.join(".git").exists())
            .expect("team-assigned task workspace")
    }
}
...
impl RuntimeAdapter for DeliveryTransport {
    ...
    fn session_start(
        ...
        let runtime_state_path = match &input.resume {
            lionclaw_runtime_api::RuntimeResume::Native { state, .. } => state.path().to_path_buf(),
            lionclaw_runtime_api::RuntimeResume::Reconstruct => std::path::PathBuf::new(),
        };
    ...
    async fn turn(
        ...
        match turn {
            ...
            DeliveryTurn::Complete | DeliveryTurn::CompleteWithOversizedReference => {
                let work = Self::writer_work(&execution);
            ...
            DeliveryTurn::Plan => {
                let proposal = planning_proposal();
                std::fs::write(
                    Self::handoff(&execution),
                    serde_json::to_vec(&serde_json::json!({
                        ...
                        "type": "plan",
                        "done": true,
                        "report": "planned after lead response",
                        "proposal": proposal,
                        "request_attention": false,
                    }))?,
                )?;
            ...
            DeliveryTurn::Validate => {
                std::fs::write(
                    Self::handoff(&execution),
                    serde_json::to_vec(&validation_handoff(&execution.input.prompt))?,
                )?;
            }
        }
        Ok(TurnResult {
...
impl RuntimeAdapter for NativeTransport {
    ...
    async fn turn(
        ...
        if execution.input.prompt.contains("## Handoff nonce") {
            ...
            .input
            .prompt
            .contains("You are an independent validator")
        {
            std::fs::write(
                handoff,
                serde_json::to_vec(&validation_handoff(&execution.input.prompt))?,
            ...
            let work = std::fs::read_dir(mission.join("tasks"))?
                .filter_map(Result::ok)
                .map(|entry| entry.path().join("work"))
                .find(|path| path.join(".git").exists())
                .expect("team-assigned task workspace");
...
fn role(id: &str, output: OutputSemantics) -> RoleInstance {
    RoleInstance {
        ...
        output,
        runtime: "codex".into(),
        instructions: format!("Production proof role {id}."),
        skills: Vec::new(),
...
fn team_for_plan(revision: u32, plan: &Plan) -> TeamRevision {
    let roles = [
        ("strategist", OutputSemantics::ProposesPlan),
        ("planner", OutputSemantics::ProposesPlan),
        ("implementer", OutputSemantics::ProducesArtifact),
        ("renamed-judgment-role", OutputSemantics::EmitsVerdict),
        ("validator", OutputSemantics::EmitsVerdict),
        ("gap-reviewer", OutputSemantics::EmitsGapVerdict),
    ]
    .into_iter()
    .map(|(id, output)| {
        let role = role(id, output);
        (role.id.clone(), role)
    })
    .collect();
    TeamRevision {
        revision,
        roles,
        planning_assignment: RoleInstanceId::new("planner").unwrap(),
        ...
        judgment_assignments: plan
            ...
            .map(|assertion| {
                (
                    assertion.id.clone(),
                    vec![RoleInstanceId::new("renamed-judgment-role").unwrap()],
                )
            })
            .collect(),
        gap_review_assignment: Some(RoleInstanceId::new("gap-reviewer").unwrap()),
        guidance: None,
    }
}
...
fn mission_proposal_with_gap(base_revision: u32, plan: Plan, gap_role: &str) -> MissionProposal {
    let mut proposal = mission_proposal(base_revision, plan);
    let team = proposal.team.as_mut().unwrap();
    let old = RoleInstanceId::new("gap-reviewer").unwrap();
    let new = RoleInstanceId::new(gap_role).unwrap();
    let mut role = team.roles.remove(&old).unwrap();
    role.id = new.clone();
    role.purpose = gap_role.replace('-', " ");
    team.roles.insert(new.clone(), role);
    team.gap_review_assignment = Some(new);
    proposal
}

fn planning_proposal() -> MissionProposal {
    let assertion = AssertionId::new("TESTS-CLEAN").unwrap();
    let plan = Plan {
        requirements: vec![Requirement {
            id: RequirementId::new("PRODUCTION-CONVERSATION").unwrap(),
            kind: RequirementKind::Validation,
        ...
        tasks: vec![Task {
            id: TaskId::new("integrate").unwrap(),
            body: "exercise the production flow".into(),
            targets: vec![assertion.clone()],
            depends_on: Vec::new(),
        }],
    };
    let mut proposal = mission_proposal(0, plan);
    proposal
        .team
        .as_mut()
        .unwrap()
        .judgment_assignments
        .insert(assertion, vec![RoleInstanceId::new("validator").unwrap()]);
    proposal
}
...
"#,
    ...
    let receipt = parked
        ...
        .unwrap_or_else(|| panic!("production retained-state failure receipt: {parked:#?}"));
    ...
    assert_eq!(
        receipt
            .final_response
            .as_ref()
            .map(|response| store.blobs().resolve(response).unwrap()),
        Some("retained-state response".into())
    );
...
async fn production_validator_and_park_compose_with_exact_awaiting_writer() {
    assert_eq!((SCHEMA_VERSION, REDUCER_VERSION), (33, 63));
    let temp = tempfile::tempdir().unwrap();
    let repo = temp.path().join("repo");
...
"#,
    ...
    std::fs::write(
        &proposal,
        serde_json::to_vec(&mission_proposal(0, awaiting_lead_validation_plan())).unwrap(),
    )
    .unwrap();
    for command in [
        cli::Cli::try_parse_from([
    ...
    let awaiting = store.require_state(&mission).await.unwrap();
    let writer_id = TaskId::new("writer-question").unwrap();
    let conversation_id = RoleInstanceId::new("implementer").unwrap();
    assert_eq!(awaiting.tasks[&writer_id].status, TaskStatus::Running);
    assert_eq!(
        awaiting.conversations[&conversation_id].lifecycle,
        lionclaw::model::ConversationLifecycle::AwaitingLead
    );
    assert!(awaiting.inflight.is_empty());
    assert_eq!(
        cli::run_with_transports(
            cli::Cli::try_parse_from([
                "lionclaw",
                ...
                repo.to_str().unwrap(),
                "--handshake",
                temp.path().join("blocked.ready").to_str().unwrap(),
            ])
            .unwrap(),
            transports.clone(),
        )
    ...
    cli::run_with_transports(
        cli::Cli::try_parse_from([
            ...
            "--repo",
            repo.to_str().unwrap(),
            "Use the production release target.",
        ])
        .unwrap(),
        transports.clone(),
    )
    .await
    .unwrap();
    let resumed_writer = tokio::spawn({
        let transports = transports.clone();
        let command = cli::Cli::try_parse_from([
            "lionclaw",
            "mission",
            ...
            repo.to_str().unwrap(),
            "--handshake",
            temp.path().join("writer-resume.ready").to_str().unwrap(),
        ])
        .unwrap();
        async move { cli::run_with_transports(command, transports).await }
    });
    tokio::time::timeout(std::time::Duration::from_secs(5), entered.acquire())
        .await
        .expect("resumed writer did not enter")
        .unwrap()
        .forget();
    release.add_permits(1);
    assert_eq!(
        resumed_writer.await.unwrap().unwrap(),
        std::process::ExitCode::SUCCESS
    );
    let driver = tokio::spawn({
        let transports = transports.clone();
    ...
    assert_eq!(active.tasks[&writer_id].status, TaskStatus::Cleared);
    let writer = &active.conversations[&conversation_id];
    assert_eq!(
        writer.lifecycle,
    ...
    assert!(matches!(
        ...
        lionclaw::model::InflightEffect::RoleTurn {
            ...
            task_id: None,
            ..
        } if role_instance.as_str() == "renamed-judgment-role"
    ));
    let validator_conversation = RoleInstanceId::new("renamed-judgment-role").unwrap();
    assert!(active.conversations.contains_key(&validator_conversation));
    ...
    assert_reference_send_rejected(
        ...
        ReferenceRejectionReason::Disallowed {
            role_instance: validator_conversation.clone(),
            output: OutputSemantics::EmitsVerdict,
        },
    )
    .await;

    let before_broadcast = store.load(&mission).await.unwrap();
    let broadcast_error = cli::run(
        cli::Cli::try_parse_from([
            "lionclaw",
            "mission",
            "send",
            ...
            "--repo",
            repo.to_str().unwrap(),
            "judgment references must reject atomically",
        ])
        .unwrap(),
    )
    .await
    .expect_err("a judgment recipient reference must fail closed");
    assert_eq!(
        broadcast_error.downcast_ref::<ReferenceRejectionReason>(),
        Some(&ReferenceRejectionReason::Disallowed {
            role_instance: validator_conversation.clone(),
            output: OutputSemantics::EmitsVerdict,
        }),
        "judgment reference rejection lost exact typed truth: {broadcast_error:#}"
    );
    assert_eq!(
        store.load(&mission).await.unwrap(),
        before_broadcast,
        "judgment reference rejection appended a message"
    );
    assert_ne!(active.deliverable_head(), base);
    ...
    assert_eq!(
        store
            ...
            .unwrap()
            .iter()
            .filter(|e| matches!(e.event, MissionEvent::RoleTurnRequested { .. }))
            .count(),
        3,
        "the exact writer may resume once, then judgment dispatches"
    );
    assert_eq!(
...
#[test]
fn renamed_terminal_gap_verdict_prompt_excludes_all_reference_prose() {
    std::thread::Builder::new()
        .name("renamed-terminal-gap-proof".into())
        ...
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                ...
                .build()
                .unwrap()
                .block_on(renamed_terminal_gap_verdict_prompt_inner());
        })
        .unwrap()
        .join()
        .unwrap();
...
async fn renamed_terminal_gap_verdict_prompt_inner() {
    let temp = tempfile::tempdir().unwrap();
    let repo = temp.path().join("repo");
    let base = initialize_repo(&repo).await;
    let fake_oci = temp.path().join("external-oci-transport");
...
"#,
    ...
    for role in [
        ...
        "planner.md",
        "implementer.md",
        "renamed-judgment-role.md",
        "validator.md",
        "renamed-closing-role.md",
    ] {
    ...
    let mission = store.list_missions().await.unwrap().pop().unwrap();
    let proposal = temp.path().join("plan.json");
    let mut mission_proposal =
        mission_proposal_with_gap(0, reference_plan(), "renamed-closing-role");
    for role in mission_proposal.team.as_mut().unwrap().roles.values_mut() {
        role.runtime = "unrelated-runtime-identity".into();
    }
    std::fs::write(&proposal, serde_json::to_vec(&mission_proposal).unwrap()).unwrap();
    for command in [
        cli::Cli::try_parse_from([
            "lionclaw",
            "mission",
    ...
    assert!(parked.conversation_is_messageable(&conversation));
    let park = parked
        .parked_effects
        .keys()
        .next()
    ...
    let terminal = captured
        ...
        .expect("real terminal RoleTurnRequest prompt");
    for producer_controlled in [
        "TERMINAL-DIRECT-PROSE",
        "TERMINAL-RECEIPT-PROSE",
        "base.txt",
    ...
    let terminal_role = store
        ...
        .find_map(|event| match event.event {
            MissionEvent::RoleTurnRequested {
                ...
                task_id: None,
                ..
            } if role_instance.as_str() == "renamed-closing-role" => Some(role_instance),
            _ => None,
        })
...
#[test]
fn production_park_then_base_move_replaces_and_isolates_stale_conversation() {
    std::thread::Builder::new()
        .name("production-park-base-move-proof".into())
        .stack_size(16 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .unwrap()
                .block_on(production_park_then_base_move_scenario());
        })
        .unwrap()
        .join()
        .unwrap();
...
fn production_park_then_base_move_scenario(
) -> std::pin::Pin<Box<impl std::future::Future<Output = ()>>> {
    Box::pin(async move {
        let temp = tempfile::tempdir().unwrap();
        let repo = temp.path().join("repo");
...
"#,
        ...
        std::fs::write(
            &proposal_path,
            serde_json::to_vec(&mission_proposal(0, reference_plan())).unwrap(),
        )
        .unwrap();
        for command in [
            cli::Cli::try_parse_from([
        ...
        let parked = MissionStore::open(&repo)
            ...
            .await
            .unwrap();
        let task_id = TaskId::new("mint-receipt").unwrap();
        let old_id = RoleInstanceId::new("implementer").unwrap();
        let old = &parked.conversations[&old_id];
        assert_eq!(old.lifecycle, lionclaw::model::ConversationLifecycle::Ready);
        let old_assignment = parked.tasks[&task_id].role_assignment.as_ref().unwrap();
        assert_eq!(old_assignment.base_sha, base);
        let old_generation = old_assignment.assignment_epoch;
        assert!(parked.tasks[&task_id].workspace_provenance.is_none());
        let old_park = parked
            .parked_effects
            .iter()
            .find(|(_, effect)| {
                matches!(
                    effect,
                    lionclaw::model::ParkedEffect::RoleTurn { task_id, .. }
                        if task_id.as_ref() == Some(&TaskId::new("mint-receipt").unwrap())
                )
            })
            .map(|(id, _)| id.clone())
        ...
        let continued = MissionStore::open(&repo)
            ...
            .await
            .unwrap();
        let continued_assignment = continued.tasks[&task_id].role_assignment.as_ref().unwrap();
        assert_eq!(continued_assignment.assignment_epoch, old_generation);
        assert_eq!(continued_assignment.base_sha, base);
        assert!(continued.tasks[&task_id].workspace_provenance.is_none());
        ...
        let MissionEvent::MessageSent { recipients, .. } = &final_events.last().unwrap().event
        else {
            panic!("--all must append one atomic message event")
        };
        assert!(recipients
            .iter()
            .any(|recipient| recipient == replacement_id));
...
"#,
    ...
    for (name, output) in [
        ...
        ("renamed-judgment-role", "emits-verdict"),
        ("validator", "emits-verdict"),
    ] {
        std::fs::write(
            root.join("roles").join(format!("{name}.md")),
...
"#,
    ...
    let role = events
        .iter()
        .position(|event| matches!(event.event, MissionEvent::RoleTurnCompleted { .. }))
        .unwrap();
    ...
    let (gate, review_effect) = events
        ...
        .find_map(|(index, event)| match &event.event {
            MissionEvent::RoleTurnRequested {
                ...
                task_id: None,
                ..
            } if role_instance.as_str() == "gap-reviewer" => Some((index, effect_id)),
            _ => None,
        })
        .unwrap();
    let review = events
...
#[test]
fn production_delivery_uses_one_exact_immutable_boundary() {
    std::thread::Builder::new()
        .name("production-delivery-proof".into())
        .stack_size(16 * 1024 * 1024)
        .spawn(|| {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .unwrap()
                .block_on(production_delivery_scenario());
        })
        .unwrap()
        .join()
        .unwrap();
...
"#,
        ...
        let task_id = TaskId::new("integrate").unwrap();
        let assignment_generation = awaiting.state.tasks[&task_id]
            .role_assignment
            .as_ref()
            .unwrap()
            .assignment_epoch;
        ...
        let events = tokio::time::timeout(std::time::Duration::from_secs(10), async {
            loop {
                ...
                if events.iter().any(|event| {
                    matches!(
                        &event.event,
                        MissionEvent::RoleTurnCompleted { outcome: Err(failure), .. }
        ...
        let request = requested_state
            ...
            .unwrap();
        let request_task = request.task_id.clone().expect("writer task");
        ...
        assert_eq!(
            state.active_role_conversation(&effect_id).unwrap().0,
        ...
        // The Slice 4 conversation-shaped workspace is also a valid checkout
        // with a real change, but team-owned task assignment is now the sole
        // workspace authority.
        let legacy_root = mission_dir
            .join("conversations")
        ...
        let active_task = active_status["tasks"]
            ...
            .unwrap()
            .iter()
            .find(|task| task["id"] == request_task.as_str())
            .unwrap();
        ...
        let task_diff = active_task["workspace_observation"]["diffstat"]
            ...
            .unwrap();
        assert!(
            task_diff.contains("exact-generation-change.txt"),
            "unexpected task diff: {task_diff}"
        );
        ...
        std::fs::remove_dir_all(&legacy_work).unwrap();
        std::fs::remove_file(legacy_root.join("observer.index")).unwrap();
        ...
        let retry_workspace = reworking.tasks[&request_task]
            .workspace_provenance
            .as_ref()
            .expect("same-generation retry retained its last settled workspace");
        assert_ne!(&retry_workspace.effect_id, retry_effect);
        let (_, _, active_workspace) = reworking
            .active_workspace_task(retry_effect)
            .expect("team assignment authorizes the active retry workspace");
        assert_eq!(&active_workspace.effect_id, retry_effect);
        assert_ne!(active_workspace.effect_id, effect_id);
        assert_eq!(active_workspace.base_sha, retry_workspace.base_sha);
        assert_eq!(
            active_workspace.assignment_epoch,
            retry_workspace.assignment_epoch
        );
        ...
        let retained_workspace = failed
            ...
            .as_ref()
            .unwrap();
        let (failed_effect_id, failed_effect) = failed.parked_effects.iter().next().unwrap();
        let lionclaw::model::ParkedEffect::RoleTurn {
            role_instance,
            task_id: Some(parked_task),
        } = failed_effect
        else {
            panic!("transport failure must park the team-assigned writer turn");
        };
        assert_eq!(role_instance, &exact_conversation_id);
        assert_eq!(parked_task, &request_task);
        assert_ne!(failed_effect_id, &retained_workspace.effect_id);
        assert_eq!(
        ...
        {
            let observed_sessions = sessions.lock().unwrap();
            assert_eq!(observed_sessions.len(), 3);
            assert!(observed_sessions[1].1);
            assert_eq!(
                observed_sessions[1].2, observed_sessions[2].2,
                "team-owned runtime state path changed between retries"
            );
            assert!(
                observed_sessions[2].1,
                "unchanged team-owned runtime state did not retain native readiness: {:#?}",
                *observed_sessions
            );
        }
        let role_resources = mission_dir
            .join("conversations")
            .join(exact_conversation_id.as_str());
        assert_eq!(
            lionclaw_runtime_api::recorded_runtime_resume_mode_at(
                active_store.lionclaw_dir(),
                role_resources.join("session-control"),
            )
            .unwrap(),
            Some(RuntimeResumeMode::Resumed),
            "team role resources: {:#?}",
            walk_paths(&role_resources)
        );
        ...
        let delivery = &completed.conversations[&exact_conversation_id];
        assert_eq!(delivery.queued.len(), 1);
        ...
        assert!(turns.lock().unwrap().is_empty());
        {
            ...
            assert_eq!(
                sessions[2].2, sessions[3].2,
                "team role state changed before interruption"
            );
            assert_eq!(
                sessions[3].2, sessions[4].2,
                "team role state changed after interruption"
            );
            assert!(!sessions[0].1);
            assert!(
                sessions[1].1,
            ...
            for session in &sessions[5..] {
                assert!(
                    !session.1,
                    "one-shot judgment roles cannot inherit writer native state"
                );
                assert_ne!(
                    session.2, sessions[4].2,
                    "one-shot judgment role reused writer state resources"
                );
            }
        }
        {
        ...
        let completions: Vec<_> = events
            ...
            .filter_map(|(index, event)| {
                matches!(event.event, MissionEvent::RoleTurnCompleted { .. }).then_some(index)
            })
            .collect();
        let oracle_completions = events
            .iter()
            ...
            .collect::<Vec<_>>();
        let (review_requested, review_effect) = events
            .iter()
            .enumerate()
            .find_map(|(index, event)| match &event.event {
                MissionEvent::RoleTurnRequested {
        ...
        let review_completed = events
            ...
            .unwrap();
        let judgment_turns = events
            .iter()
            .enumerate()
            .filter_map(|(requested, event)| match &event.event {
                MissionEvent::RoleTurnRequested {
                    role_instance,
                    effect_id,
                    task_id: None,
