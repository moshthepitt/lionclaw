...
scenario_fix_bug() {
    ...
    for i in $(seq 1 "$RUNS"); do
        ...
        local mid; mid="$(mission_json start --plugin "$PLUGIN" --repo "$repo" \
...
JSON
        "$BIN" mission submit-plan "$mid" --repo "$repo" --plugin "$PLUGIN" --plan "$repo/plan.json" >/dev/null 2>&1
        timeout 900 "$BIN" mission advance "$mid" --repo "$repo" --plugin "$PLUGIN" >/dev/null 2>&1
